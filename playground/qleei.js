...
const mem = {
  ...
  alloc_js_str_as_zstr(js_str) {
    if (typeof js_str != 'string') {
      throw new TypeError('alloc_js_str_as_zstr expected a javascript string but got ' + (typeof js_str));
    }
    const binary = Utf8.encode(js_str);
    const ptr = mem.alloc(binary.byteLength + 1);
    if (ptr === 0) {
      throw new Error('Not enough memory to allocate JavaScript string');
    }
    const view = new Uint8Array(mem.buffer, ptr);
    for (let i = 0; i < binary.byteLength; ++i) {
      view[i] = binary[i];
    }
    view[binary.byteLength] = 0;
    return ptr;
  },
...
const env = {
  ...
  qleei_wasm_printf(pFmt, pVargs) {
    ...
    for (let i = 0; i < fmt.length; ++i) {
      ...
      if (ss.startsWith('%p')) {
        i++;
        const p = view.shift_ptr();
        print('0x' + p.toString(16).padStart(8, '0'));
        continue;
      }
...
export async function load_interpreter() {
  ...
  const interpret_code = async (code) => {
    ...
    const view = new Uint8Array(mem.buffer, code_ptr, bytes.byteLength);
    for (let i = 0; i < CODE_BUF_CAP; ++i) {
      const value = i < bytes.byteLength ? bytes[i] : 0;
      view[i] = value;
    }
